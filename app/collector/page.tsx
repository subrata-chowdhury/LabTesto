...
const CollectorDashboard = () => {
  ...
  const [orderReportDeliveryData, setOrderReportDeliveryData] = useState<
    Order[]
  >([]);
  const [outForSampleCollectionData, setOutForSampleCollectionData] = useState<
    Order[]
  >([]);
  const [outForReportDeliveryData, setOutForReportDeliveryData] = useState<
    Order[]
  >([]);
  ...
  const fetchOrders = useCallback(async () => {
    ...
    const res = await fetcher.get<{
      ...
      `/collector/orders?filter=${JSON.stringify(filterData)}&limit=999&page=1`,
    );
    ...
    if (res.body) {
      ...
      const upcomingOrders = res.body.orders.filter((order) => {
        return (
          ...
          new Date(order.sampleTakenDateTime?.start || "") > endOfDay
        );
      });
      ...
      const todaysOrders = res.body.orders.filter((order) => {
        return (
          order.status === "Ordered" &&
          new Date(order.sampleTakenDateTime?.start || "") >= startOfDay &&
          new Date(order.sampleTakenDateTime?.start || "") <= endOfDay
        );
      });
      ...
      setOrderReportDeliveryData(
        res.body.orders.filter((order) => {
          return (
            order.status === "Report Generated" &&
            new Date(order.reportDeliverTime.date.start || "") <= endOfDay
          );
        }),
      );
      setOutForSampleCollectionData(
        res.body.orders.filter((order) => {
          return order.status === "Out for Sample Collection";
        }),
      );
      setOutForReportDeliveryData(
        res.body.orders.filter((order) => {
          return order.status === "Out for Report Delivery";
        }),
      );
      setSampleCollectedData(
        res.body.orders.filter((order) => {
  ...
  return (
    <>
      {outForSampleCollectionData.length > 0 && (
        <>
          <h1 className="text-xl font-semibold mx-4 mb-2 mt-4">
            Out For Sample Collection
          </h1>
      ...
      {sampleCollectedData.length > 0 && (
        <>
          <h1 className="text-xl font-semibold mx-4 mb-2 mt-4">
            Sample to be Delivered to Lab
          </h1>
          <div className="flex flex-col gap-2 mx-4">
            {sampleCollectedData.map((order) => (
              <div
                key={order._id}
                className="flex justify-between items-center flex-col sm:flex-row gap-2 rounded-md border-2 border-gray-300/50 bg-white p-2 px-3"
              >
                {/* <div>{order._id}</div> */}
                ...
                <div className="flex gap-2">
                  <Link
                    className="px-2.5 py-1 bg-primary text-white rounded text-sm font-medium"
                    href={"/collector/orders/view/" + order._id}
                  >
                    View
                  </Link>
                  <Link
                    className="px-2.5 py-1 bg-primary text-white rounded text-sm font-medium"
                    href={"/collector/orders/edit/" + order._id}
                  >
                    Edit
                  </Link>
                </div>
      ...
      {upComingOrderData.length <= 0 && orderReportDeliveryData.length <= 0 && (
      ...
      <div className="grid grid-rows-2 h-full mx-4">
        ...
        {orderReportDeliveryData.length > 0 && (
          <div>
            <h2 className="text-lg mb-1.5 font-medium">
