...
function Menubar() {
    ...
    return (
        <>
            ...
            {isOpen && <div className='sm:relative shadow-2xl sm:shadow fixed left-0 top-0 bg-white dark:bg-black flex flex-col gap-2 z-20 px-2 sm:px-4 py-4 sm:py-6 h-full transition-all' onMouseEnter={() => setMenubarHovered(true)} onMouseLeave={() => { if (window.innerWidth > 640) setMenubarHovered(false) }}>
                ...
                <Link className={"cursor-pointer flex justify-start gap-3 items-center mb-2 rounded-lg text-primary dark:text-white" + (menubarHovered ? " mx-1.5" : " mx-auto")} href='/collector/profile' aria-label="View Profile">
                    <UserIcon size={40} className="rounded-full p-2 bg-primary/20 dark:bg-white/20" />
                ...
                <div className=''>
                    <div
                        ...
                        <Image src={logout} alt='' className='filter brightness-0 dark:invert' width={20} height={20} style={{ width: 20, height: 20 }} />
...
const Menu: React.FC<MenuProps> = ({ name, link, active, image, menubarHovered }) => {
    return (
        <Link href={link}>
            <div className={`flex justify-start gap-3 p-3 items-center rounded-lg transition-all ${active ? 'bg-primary dark:bg-white/20 text-white' : ''}`}>
                <Image src={image} alt='' width={24} height={24} className={'w-5 h-5 sm:w-6 sm:h-6 filter brightness-0 ' + (active ? "invert" : "dark:invert")} />
