...
export const OrderDetailsCard = ({ order, onPass }: { order: Order, onPass: (order: Order) => void }) => {
    return (
        <div key={order._id} className='flex justify-between items-center flex-col sm:flex-row gap-2 rounded-md border-2 border-gray-300/50 dark:border-white/25 bg-white dark:bg-black dark:shadow-md dark:shadow-black p-2 px-3'>
            {/* <div>{order._id}</div> */}
            ...
            <div className='flex gap-2'>
                <Link className='px-2.5 py-1 bg-primary dark:bg-white/20 text-white rounded text-sm font-medium' href={('/collector/orders/view/' + order._id)}>View</Link>
                <Link className='px-2.5 py-1 bg-primary dark:bg-white/20 text-white rounded text-sm font-medium' href={('/collector/orders/edit/' + order._id)}>Edit</Link>
                {order.status === 'Ordered' && <button
                    className='px-2.5 py-1 bg-orange-500 text-white rounded text-sm font-medium cursor-pointer'
                    onClick={async () => {
                        if(!window.confirm('Are you sure you want to Pass this order to another collector?')) return;
