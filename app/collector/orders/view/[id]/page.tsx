  ...
  return (
    <div className="flex-1 flex flex-col gap-4 pt-2">
        ...
        <div className="bg-white dark:bg-black rounded shadow-md" key={index}>
          <div className="flex justify-between p-3 px-4">
            <div className="flex flex-col gap-2 justify-between h-full">
              ...
              <div className="mt-auto font-medium text-xl">
            ...
            {order.items[index]?.patientDetails?.length > 0 &&
              Array(item.quantity)
                ...
                .map((_, i) => (
                  ...
                  >
                    {order.items[index]?.patientDetails[i]?.name
                      ?.split(" ")
                      .map((e) => e.charAt(0))
                      .join("") || "Add +"}
                  </div>
      ...
      <Link
        ...
        className="ms-auto px-4 py-2 bg-primary dark:bg-white/15 text-white rounded font-medium"
      >
        Edit Order
      </Link>
      ...
      <div className="bg-white dark:bg-black px-6 py-4 rounded">
        ...
        <div>
          ...
          {new Date(order?.sampleTakenDateTime?.end || "").toDateString()},{" "}
          {
            new Date(order?.sampleTakenDateTime?.end || "")
              .toTimeString()
              .split(" ")[0]
          }
        </div>
      ...
      {!(
        ...
        <div className="bg-white dark:bg-black px-6 py-4 rounded">
          ...
          <div>
            <span className="font-medium">Name:</span> {order?.collector?.name}
          </div>
          <div>
      ...
      <div className="bg-white dark:bg-black px-6 py-4 rounded">
        ...
        <div>
          <span className="font-medium">Pin:</span> {order?.address?.pin}
        </div>
        <div>
    ...
    <Model heading="Patient Details" onClose={onClose}>
      <div className="px-7 pb-6 py-4 grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div className="font-normal">
          Name: <span className="font-semibold">{patientDetails?.name}</span>
        </div>
        <div className="font-normal">
          Age: <span className="font-semibold">{patientDetails?.age}</span>
        </div>
        <div className="font-normal">
          Gender:{" "}
          <span className="font-semibold">{patientDetails?.gender}</span>
        </div>
      </div>
    </Model>
  );
