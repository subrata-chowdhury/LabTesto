...
const Page = () => {
  ...
  const maxPrice = orderDetails.items.reduce(
  ...
  const handleSave = async () => {
    ...
    else toast.error(res.error || "Error updating order");
  };
  ...
  return (
    <div className="bg-white dark:bg-black mt-4 p-8 px-10">
      <div className="text-xl flex gap-3 items-center font-bold pb-6">
        ...
        <Title
          title={
            <p className="text-nowrap font-medium dark:text-black">
              i. You can only change status on given order
              <br />
              ii. And one at a time
            </p>
          }
        >
      ...
      <Input
        ...
        onChange={(val) =>
          setOrderDetails({
            ...orderDetails,
            paid: Math.min(Number(val), maxPrice),
          })
        }
      />
      <div className="pb-4 flex justify-between font-semibold mt-6 pt-5 border-t-2 border-gray-300/50">
