...
const Orders = () => {
  ...
  const fetchOrders = useCallback(async () => {
    ...
    const res = await fetcher.get<{
      ...
      `/collector/orders?filter=${JSON.stringify(filterData)}&limit=${limit}&page=${currentPage}`,
    );
    ...
    if (res.body) {
      ...
      setAnalytics((prevVal) => ({
        ...prevVal,
        totalOrders: res.body ? res.body.pagination.totalOrders : 0,
      }));
    }
    setLoading(false);
                ...
                component: ({ data }) => (
                  <div className="flex gap-1 items-center w-fit">
                    <button
                      ...
                      onClick={() =>
                        navigate.push("/collector/orders/view/" + data._id)
                      }
                    >
                      View
                    </button>
                    ...
                    <button
                      className="text-blue-500"
                      onClick={() =>
                        navigate.push(`/collector/orders/edit/${data._id}`)
                      }
                    >
                      Edit
                    </button>
