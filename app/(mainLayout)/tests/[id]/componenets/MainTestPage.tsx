    ...
    const cartItem = {
      product: {
        ...
        price: (
          ...
          labBaseDetails.price * (labBaseDetails.offer / 100)
        ).toFixed(2),
      },
      quantity: 1,
    };
  ...
  return (
    <div className="bg-black/0 dark:bg-white/0 p-1 md:py-9 md:px-10">
      <section className="bg-white dark:bg-black border-2 border-gray-300/50 dark:border-white/15 shadow-md shadow-primary/5 p-7 px-8 flex flex-col rounded-lg">
        ...
        {(testDetails?.otherTerms || []).length > 0 && (
          <div className="text-sm text-gray-500 pt-2">
            <strong className="font-normal">Also known as:</strong>{" "}
            <span className="text-black font-medium">
              {testDetails?.otherTerms?.join(", ")}
            </span>
          </div>
        )}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-6">
          <div className="flex gap-4">
            ...
            <p className="text-gray-500 dark:text-gray-400">
              {testDetails.sampleType}
            </p>
          </div>
          <div className="flex gap-4">
            <p className="font-medium flex gap-2">
              <TubeIcon />
              {testDetails.tubeType.includes("Tube")
                ? "Tube"
                : "Container"}{" "}
              Type
            </p>
            <p className="text-gray-500 dark:text-gray-400">
              {testDetails.tubeType}
            </p>
          </div>
          {(testDetails?.fastingRequired?.length || 0) > 0 && (
            <div className="flex gap-4">
              <p className="font-medium flex gap-2">
                <FoodIcon />
                Fasting Required
          ...
          {labs.length > 0 && (
            <div className="flex gap-4 items-center">
              ...
              <Title
                title={
                  <p className="text-nowrap font-medium">See lab Details</p>
                }
        ...
        <div className="bottom-0 flex flex-col sm:flex-row gap-4 items-center justify-between mt-6">
            ...
            {labs.length > 0 && (
              <>
                <div className="text-2xl font-semibold">
                  ₹{labBaseDetails.offer.toFixed(2)}
                </div>
                <div className="text-base line-through text-gray-500 dark:text-gray-400">
                  ₹{labBaseDetails.price}
                </div>
                <div className="text-sm font-semibold text-red-400">
                  {(
                    ((labBaseDetails.price - labBaseDetails.offer) /
                      labBaseDetails.price) *
                    100
                  ).toFixed(2)}
                  % OFF
          ...
          <div className="flex gap-3">
            {labs.length > 0 && (
              <button
                disabled={loading}
                className="px-5 py-2 rounded-md bg-primary dark:bg-white/15 text-white font-medium cursor-pointer"
                onClick={onBook}
              >
            ...
            {labs.length > 0 && (
              <button
                disabled={loading}
                className="px-5 py-2 rounded-md bg-primary dark:bg-white/15 text-white font-medium text-nowrap cursor-pointer"
                onClick={onAddToCart}
              >
      ...
      <section className="mt-1 md:mt-4 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-1 md:gap-5 rounded-lg">
        ...
        {labs.length > 0 &&
          labs.map((labObj, i) => (
            <div
              key={i}
              className="border-2 border-gray-300/50 dark:border-white/15 shadow-md shadow-primary/5 p-5 py-4 rounded-lg cursor-pointer flex justify-between bg-white dark:bg-black"
            ...
            >
              <div className="flex items-center gap-3">
                <div className="w-14 h-14 bg-primary/20 dark:bg-white/30 rounded-md flex items-center justify-center">
                  {labObj.image && (
                    <Image src={labObj.image} width={56} height={56} alt="" />
                  )}
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="text-sm font-semibold text-red-400">
                      {(
                        ((labObj.price - (labObj?.offer || 0)) / labObj.price) *
                        100
                      ).toFixed(2)}
                      % OFF
                    </div>
      ...
      {showOrderPopup && (
        <section className="w-screen h-screen z-20 flex flex-col fixed top-0 left-0 bg-gray-100">
          <div
            className="ms-auto mr-6 mt-7 cursor-pointer"
    ...
    return (
      <section className="mt-1 md:mt-4 py-8 px-8 rounded-lg border-2 border-gray-300/50 dark:border-white/15 bg-white dark:bg-black shadow-md shadow-primary/5">
          ...
          {labBaseDetails.packages.length > 0 && (
            <div className="grid grid-flow-col justify-start gap-2">
              ...
              <div className="flex flex-col gap-1 w-full">
                ...
                <ul
                  className={
                    "list-disc list-inside relative w-full " +
                    (!seeMore.packagesInclude
                      ? "max-h-[4.8rem] overflow-y-hidden"
                      : "")
                  }
                  onClick={() =>
                    setSeeMore({
                      ...seeMore,
                      packagesInclude: !seeMore.packagesInclude,
                    })
                  }
                >
                ...
                {labBaseDetails.packages.length > 5 && (
                  <div
                    className="mt-auto text-sm font-semibold cursor-pointer text-primary dark:text-white"
                    onClick={() =>
                      setSeeMore({
                        ...seeMore,
                        packagesInclude: !seeMore.packagesInclude,
                  ...
                  >
                    See {!seeMore.packagesInclude ? "More" : "Less"}
                  </div>
                )}
              </div>
            </div>
          )}
          {(testDetails?.overview?.length || 0) > 0 && (
            <div className="grid grid-flow-col justify-start gap-2">
              <DescriptionIcon />
              <div className="flex flex-col gap-1">
                ...
                <div
                  className={
                    "text-gray-500 dark:text-gray-300 relative " +
                    (!seeMore.overview
                      ? "max-h-[4.8rem] overflow-y-hidden"
                      : "")
                  }
                  onClick={() =>
                    setSeeMore({ ...seeMore, overview: !seeMore.overview })
                  }
                >
                  <div
                    className="tiptap"
                    style={{ padding: 0, border: 0, minHeight: "auto" }}
                    dangerouslySetInnerHTML={{
                      __html: testDetails.overview || "",
                    }}
                  ></div>
                </div>
                {(testDetails?.overview || "").replace(/<[^>]*>/g, "").length >
                  50 && (
                  <div
                    className="mt-auto text-sm font-semibold cursor-pointer text-primary dark:text-white"
                    onClick={() =>
                  ...
                  >
                    See {!seeMore.overview ? "More" : "Less"}
                  </div>
                )}
              </div>
            </div>
          )}
          {(testDetails?.description || "").replace(/<[^>]*>/g, "").length >
            0 && (
              ...
              <div className="flex-1 flex flex-col gap-1">
                ...
                <div
                  ...
                  onClick={() =>
                    setSeeMore({
                      ...seeMore,
                      description: !seeMore.description,
                    })
                  }
                >
                  <div
                    className="tiptap text-gray-500 dark:text-gray-300"
                    style={{ padding: 0, border: 0, minHeight: "auto" }}
                    dangerouslySetInnerHTML={{
                ...
                {(testDetails?.description || "").replace(/<[^>]*>/g, "")
                  .length > 50 && (
                  <div
                    className="mt-auto text-sm font-semibold cursor-pointer text-primary dark:text-white"
                    onClick={() =>
                  ...
                  >
                    See {!seeMore.description ? "More" : "Less"}
                  </div>
                )}
              </div>
            </div>
          )}
          {(testDetails?.testResultInterpretation || "").length > 0 && (
            <div className="grid grid-flow-col justify-start gap-2">
              <DescriptionIcon />
              <div className="flex flex-col gap-1 overflow-x-auto">
                ...
                <div
                  className={
                    "text-gray-500 dark:text-gray-300 relative " +
                    (!seeMore.testResultInterpretation
                      ? "max-h-[4.8rem] overflow-y-hidden"
                      : "")
                  }
                  onClick={() =>
                ...
                {(testDetails.testResultInterpretation || "").replace(
                  /<[^>]*>/g,
                  "",
                ).length > 50 && (
                  <div
                  ...
                  >
                    See {!seeMore.testResultInterpretation ? "More" : "Less"}
                  </div>
                )}
              </div>
            </div>
          )}
          {(testDetails?.riskAssesment || "").replace(/<[^>]*>/g, "").length >
            0 && (
            <div className="grid grid-flow-col justify-start gap-2">
              <DescriptionIcon />
              <div className="flex flex-col gap-1">
                ...
                <div
                  ...
                  onClick={() =>
                    setSeeMore({
                      ...seeMore,
                      riskAssesment: !seeMore.riskAssesment,
                    })
                  }
                >
                  <div
                ...
                {(testDetails.riskAssesment || "").replace(/<[^>]*>/g, "")
                  .length > 50 && (
                  <div
                    className="mt-auto text-sm font-semibold cursor-pointer text-primary dark:text-white"
                    onClick={() =>
                  ...
                  >
                    See {!seeMore.riskAssesment ? "More" : "Less"}
                  </div>
                )}
              </div>
            </div>
        ...
        {labBaseDetails.ranges.length > 0 && (
          <div className="mt-1 md:mt-4 flex flex-col gap-5 rounded-lg">
            {labBaseDetails.ranges.length > 0 && (
...
const LabDetailsSidePopup = memo(
  ({ labId, onClose }: { labId: string; onClose: () => void }) => {
    ...
    return (
      <section
        className="fixed w-screen h-screen top-0 left-0 bg-black/20 dark:bg-white/20 backdrop-blur-sm z-50"
        onClick={() => onClose()}
      >
        <div
          className="fixed left-0 top-0 w-[90vw] sm:w-[70vw] bg-white dark:bg-black shadow-2xl"
          onClick={(e) => e.stopPropagation()}
        >
          <button
            className="flex justify-end p-2 absolute right-0 top-0 cursor-pointer translate-x-full bg-white dark:text-white dark:bg-black rounded-e-md border-2 border-transparent"
            onClick={() => onClose()}
          >
            <CrossIcon />
          </button>
          <div className="overflow-y-scroll h-screen">
            <div className="bg-primary/20 dark:bg-white/15 relative min-h-52 w-full overflow-hidden p-4">
              {lab?.image && (
                <Image
                  src={lab.image}
                  className="bg-white/80 z-20 relative rounded"
                  alt=""
                  width={80}
                  height={80}
                />
              )}
              <h2 className="font-bold text-2xl bottom-16 absolute z-20 text-white">
                {loading ? (
                  <div className="h-6 w-28 animate-pulse bg-gray-300 rounded"></div>
                ) : (
                  lab?.name
                )}
              </h2>
              <div className="font-medium flex gap-1 bottom-10 absolute z-20 text-white">
                {loading ? (
                  <div className="h-4 w-52 animate-pulse bg-gray-300 rounded"></div>
                ) : (
                  <>
              ...
              {loading ? (
                <div className="h-6 w-36 animate-pulse absolute bottom-3 bg-gray-300 rounded z-20"></div>
              ) : (
                <div className="flex gap-2 absolute bottom-4 z-20">
                  {Array.from({ length: Math.floor(lab?.rating || 0) }).map(
                    (_, i) => (
                      <Image
                        key={i}
                        src={filledStar}
                  ...
                  {Array.from({ length: 5 - Math.floor(lab?.rating || 0) }).map(
                    (_, i) => (
