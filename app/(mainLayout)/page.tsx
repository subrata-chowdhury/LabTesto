...
function FrequentRequiredTests() {
  const [ref, isVisible] = useIsVisible<HTMLSelectElement>({
    threshold: 0.4,
    once: true,
  });
  ...
  return (
    <section ref={ref} className="mx-auto flex flex-col py-20">
      <h1
        className={
          "text-2xl sm:text-3xl md:text-4xl font-bold text-center px-4 mx-auto mt-4 " +
          (isVisible
...
function SlideImage({
  ...
  return (
    <>
      <Image
        className="h-96 mx-auto object-scale-down dark:hidden block"
        src={imgLight}
        alt=""
      />
      <Image
        className="h-96 mx-auto object-scale-down hidden dark:block"
        src={imgDark}
        alt=""
      />
    </>
...
export function Promises({ heading }: { heading?: string }) {
  ...
  return (
    <section ref={ref} className="mx-auto md:w-[95%] flex flex-col py-20">
      <h1
        className={
          "text-2xl sm:text-3xl md:text-4xl font-bold text-center mx-auto mt-4 " +
          (isVisible
            ? " opacity-100 translate-y-0"
            : " opacity-0 translate-y-6")
        }
...
function Card({
  ...
  return (
    <div
      className={`shadow-primary/5 flex flex-col p-4 pb-2 bg-white dark:bg-black rounded-md shadow-lg dark:shadow-black border border-primary/20 min-w-52 ${className}`}
      style={style}
    >
      <h1 className="text-xl font-semibold mb-1.5 text-primary dark:text-white">
        {label}
      </h1>
      {tag && (
      ...
      {description && (
        <p className="text-sm opacity-70 mb-2.5">{description}</p>
      )}
      {icon && <Image src={icon} alt="" style={{ width: 120 }} />}
      <Link
        href={link?.href || "#"}
        className="border-t border-gray-400 mt-auto pt-2 text-primary dark:text-white text-sm flex items-center gap-2"
      >
        {link?.label}
        <LinkArrowIcon size={12} />
      </Link>
    </div>
  );
}
    ...
    <div className={`rounded-md p-0 py-4 sm:p-4 ${className}`} style={style}>
      <div className="flex flex-col gap-3">
        <div
          ...
          className="w-24 h-24 p-4 mx-auto rounded-3xl flex items-center justify-center"
        >
          {icon && <Image src={icon} alt="" className="mx-auto" width={120} />}
        </div>
        <h1 className="sm:text-lg font-medium text-center">{label}</h1>
        {subText && (
          <p className="text-sm text-primary text-opacity-45">{subText}</p>
        )}
...
function CardType3({
  ...
  return (
    <a
      href={link}
      className={`shadow-md group overflow-hidden relative transition-all shadow-primary/5 bg-white dark:bg-black dark:shadow-black rounded-md px-8 py-6 border border-primary/20 ${className}`}
      style={style}
    >
      ...
      <li className="flex flex-col items-center">
        ...
        <span className="text-primary dark:text-white text-center group-hover:text-gray-100 transition-all duration-300">
          {label}
        </span>
      </li>
    </a>
...
function CardType4({
  ...
  return (
    <div
      ref={ref}
      className={
        "inline-flex dark:bg-black w-full h-32 mr-3 mb-3 flex-col items-center justify-center rounded-lg border border-primary/20 dark:border-white/25 shadow-md shadow-primary/10 " +
        className
      }
      style={style}
    >
      {icon && (
        <figure>
          <Image src={icon} alt="" className="sr-icon" width={50} height={50} />
        </figure>
      )}
      <span className="text-primary dark:text-white text-4xl sm:text-5xl font-bold">
