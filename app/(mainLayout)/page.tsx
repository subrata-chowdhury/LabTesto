...
function Card({ label = "CBC", tag, description, link, icon, className, style }: { label: string, tag?: string, description?: string, link?: { label: string, href: string }, icon?: StaticImageData, className?: string, style?: React.CSSProperties }) {
    return (
        <div className={`shadow-indigo-100 flex flex-col bg-white dark:bg-[#172A46] rounded-md shadow-lg dark:shadow-black border border-primary/45 min-w-52 ${className}`} style={style}>
            <div className="p-4 pb-2">
                <h1 className="text-xl font-semibold mb-1.5 text-primary">{label}</h1>
                {tag && <p className="text-xs font-semibold text-white mb-2.5 bg-primary bg-opacity-20 px-4 py-1.5 w-fit rounded-full">{tag}</p>}
...
function CardType3({ label = "Cardiology", link = "#", icon, className, style }: { label: string, link?: string, icon?: StaticImageData, className?: string, style?: React.CSSProperties }) {
    return (
        <a href={link} className={`shadow-md group overflow-hidden relative transition-all shadow-indigo-100 bg-white dark:bg-[#172A46] dark:shadow-black rounded-md px-8 py-6 border border-primary/40 ${className}`} style={style}>
            <div className='bg-primary -z-10 transition-all duration-500 w-0 h-0 top-0 left-0 rounded-full group-hover:w-[150%] group-hover:h-[400px] absolute group-hover:-top-[25%] group-hover:-left-[25%]'></div>
            <li className="flex flex-col items-center">
                <figure>
                    {icon && <Image
                        src={icon}
                        alt=""
                        className="sr-icon filter group-hover:brightness-0 group-hover:invert"
                        width={80}
                        height={80}
                    />}
                </figure>
                <span className="text-primary text-center group-hover:text-gray-100 transition-all duration-300">{label}</span>
            </li>
        </a>
    )
}
...
function CardType4({ label = "CBC", value, subText, icon, className, style }: { label: string, value: number, subText?: string, description?: string, icon?: StaticImageData, className?: string, style?: React.CSSProperties }) {
    ...
    return (
        <div ref={ref} className={"inline-flex dark:bg-[#172A46] dark:shadow-md w-full h-32 mr-3 mb-3 flex-col items-center justify-center rounded-lg border border-primary/40 shadow-md shadow-primary/10 " + className} style={style}>
            {icon && <figure>
                <Image
                    src={icon}
                    alt=""
            ...
            <span className="text-primary text-4xl sm:text-5xl font-bold">{realValue >= value ? label : (realValue == 0 ? label : realValue)}</span>
