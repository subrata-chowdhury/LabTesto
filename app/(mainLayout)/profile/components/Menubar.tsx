    ...
    return (
        <div className='text-sm border-b-2 pb-2 sm:border-b-0 sm:pb-0 pr-1.5 sm:pr-5 lg:pr-12 sm:text-base flex sm:flex-col gap-2'>
            {[
                ...
                <Link key={item.href} href={item.href} className={`bg-primary font-medium ${currentPath === item.href ? 'bg-opacity-15 text-primary' : 'bg-opacity-0 text-gray-500 dark:text-gray-400'} h-fit text-[10px] sm:text-base px-4 sm:px-6 py-1.5 rounded`}>{item.label}</Link>
            ))}
            {/* <div
