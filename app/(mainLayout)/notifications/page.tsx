      ...
      if (response.body) {
        ...
        setNotifications(
          sortedOrders.map((e) => ({
            ...
            subText: (
              <p>
                Collector is {e.status} for{" "}
                {e.items.map((e) => e.product.test.name).join(", ")}. Be prepare
              </p>
            ),
        ...
        {notifications.map((notification, outerIndex) => (
          ...
          >
            <div className="flex-1">
              <div
                className={`font-semibold ${notification.type === "warning" ? "text-orange-400" : notification.type === "success" ? "text-green-400" : notification.type === "danger" ? "text-red-400" : "text-primary"}`}
              >
