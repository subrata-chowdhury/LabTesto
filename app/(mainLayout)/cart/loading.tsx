    ...
    <ul className="space-y-4 flex-1 max-h-[70vh] overflow-y-scroll pb-5">
      {Array(3)
        ...
        .map((_, index) => (
          <li
            key={index}
            className="bg-white dark:bg-black rounded shadow-md flex flex-col"
          >
            ...
            <div className="bg-primary/5 dark:bg-white/5 p-1 text-xs">
              {Array(3)
                ...
                .map((_, i) => (
                  <div
                    className="w-12 h-6 inline-flex mr-2 m-1 mb-0 bg-primary text-white dark:bg-white/20 animate-pulse rounded-full"
                    key={i}
                  ></div>
                ))}
            </div>
