...
const Page = () => {
  ...
  async function saveLab() {
    setLoading(true);
    const res = await fetcher.post<LabAboutDetails, { messege: string }>(
      `/admin/labs/${id}`,
      {
        ...labDetails,
        description: labDetails.tempDescription || labDetails.description || "",
      },
    );
    if (res.status === 200) {
  ...
  return (
    <>
      <AboutForm
        ...
        loading={loading}
        onChange={(labDetails) => setLabDetails(labDetails)}
        onSave={saveLab}
      />
    </>
  );
