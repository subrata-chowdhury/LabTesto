...
export default function RangePopup({
  rangeDetails,
  onClose = () => {},
  onSave = () => {},
}: {
  rangeDetails: { test: string; ranges: { [key: string]: string }[] };
  onClose?: () => void;
  onSave?: (rangeDetails: {
    name?: string;
    test: string;
  ...
  const [headers, setHeaders] = useState<string[]>(
    Object.keys(rangeDetails.ranges[0] || {}) || [],
  );
  const [showRangePopup, setShowRangePopup] = useState<{
    index: number;
  } | null>(null);
    ...
    <Model heading="Add Range" onClose={onClose}>
      <div className="px-7 py-4 pb-2 text-sm flex flex-col gap-3">
        ...
        {headers.length > 0 && (
          <>
            <div
              ...
              onClick={() =>
                setShowRangePopup({ index: rangeData.ranges.length })
              }
            >
              <div>New Entry</div>
        ...
        <div className="p-5 px-0 ms-auto justify-end items-end flex gap-4">
          <div
            className="font-medium text-blue-500 h-10 flex justify-center items-center px-4 border-2 border-blue-400 rounded cursor-pointer"
            onClick={onClose}
          >
            Cancel
          </div>
      ...
      {showRangePopup && (
        <Model heading="Add Range" onClose={() => setShowRangePopup(null)}>
          <div className="px-7 py-4 pb-2">
            <div className="grid grid-cols-1 gap-4 text-sm">
              <TagInput
                label="Values *"
                values={
                  rangeData.ranges[showRangePopup.index]
                    ? Object.values(rangeData.ranges[showRangePopup.index])
                    : []
                }
                onChange={(vals) => {
            ...
            <div className="p-5 px-0 ms-auto justify-end items-end flex gap-4">
              <div
                className="font-medium text-blue-500 h-10 flex justify-center items-center px-4 border-2 border-blue-400 rounded cursor-pointer"
                onClick={() => setShowRangePopup(null)}
              >
                Cancel
              </div>
              <div
