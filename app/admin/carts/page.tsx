...
const Carts = () => {
  ...
  const [analytics, setAnalytics] = useState<{ totalCarts: number }>({
    totalCarts: 0,
  });
  ...
  const fetchCarts = useCallback(async () => {
    ...
    const res = await fetcher.get<{
      ...
      `/admin/carts?filter=${JSON.stringify(filterData)}&limit=${limit}&page=${currentPage}`,
    );
                ...
                component: ({ data }) => (
                  <div className="flex gap-1 items-center w-fit">
                    ...
                    <button
                      ...
                      onClick={() => deleteCart(data._id as string)}
                    >
                      <TrashBinIcon />
                    </button>
                  </div>
