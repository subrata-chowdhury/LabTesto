...
const Users = () => {
  ...
  const [analytics, setAnalytics] = useState<{
    ...
    clients: number;
    admins: number;
  }>({ totalUsers: 0, clients: 0, admins: 0 });
  ...
  const fetchUsers = useCallback(async () => {
    ...
    if (type === "Clients") {
      ...
      const res = await fetcher.get<{
        ...
        `/admin/users?filter=${JSON.stringify(filterData)}&limit=${limit}&page=${currentPage}`,
      );
      ...
      const res = await fetcher.get<{
        ...
        `/admin/users/admins?filter=${JSON.stringify(filterData)}&limit=${limit}&page=${currentPage}`,
      );
      if (res.status !== 200) return;
      if (res.body) {
        setUserData(res.body.users);
  ...
  async function deleteUser(id: string) {
    ...
    const res =
      type === "Clients"
        ? await fetcher.delete(`/admin/users/${id}`)
        : await fetcher.delete(`/admin/users/admins/${id}`);
    if (res.status !== 200) return;
    await fetchUsers();
  }
                ...
                component: ({ data }) => (
                  <div className="flex gap-1 items-center w-fit">
                    ...
                    <button
                      ...
                      onClick={() => deleteUser(data._id as string)}
                    >
                      <TrashBinIcon />
                    </button>
                  </div>
