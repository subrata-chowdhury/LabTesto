    ...
    return (
        <>
            <div className='flex flex-col'>
                ...
                <div className='ms-auto mb-4 flex gap-2 bg-primary dark:bg-white/15 text-white px-4 py-2 rounded cursor-pointer' onClick={() => toast.warn('Feature not available yet')}>
                    <div>New User</div>
                        ...
                        config: [
                            ...
                            { heading: 'Created At', selector: 'createdAt', component: ({ data }) => <div>{new Date(data.createdAt).toDateString()}</div> },
                            {
                                heading: 'Actions', component: ({ data }) => <div className='flex gap-1 items-center w-fit'>
                                    <button className='text-blue-500 cursor-pointer' onClick={() => toast.warn('Feature not available yet')}>View</button>|
                                    <button className='text-blue-500 cursor-pointer' onClick={() => toast.warn('Feature not available yet')} >Edit</button>|
                                    <button className='text-[#ff5d76] cursor-pointer' onClick={() => deleteUser(data._id as string)} ><TrashBinIcon /></button>
                                </div>
                            }
                        ],
                        data: userData
