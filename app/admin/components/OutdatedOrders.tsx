...
export default function OutdatedOrders() {
  ...
  return (
    <div className="mt-5 bg-white dark:bg-black px-5 py-4 rounded-lg border-2 border-gray-300/50 dark:border-white/20 w-fit">
      <div className="text-lg font-semibold pb-2.5 border-b-2 border-gray-300/50">
        Outdated Orders
      </div>
      <div className="w-full flex flex-col gap-2 overflow-x-auto mt-3 min-w-96 h-60 overflow-y-auto">
        {outdatedOrders.filter((order) => order.status === "Ordered").length >
          0 && <h3 className="text-base">Ordered</h3>}
        {outdatedOrders
          .filter((order) => order.status === "Ordered")
          .map((order) => (
            <OutdatedOrderCard order={order} key={order._id} />
          ))}
        {outdatedOrders.filter(
          (order) => order.status === "Out for Sample Collection",
        ).length > 0 && <h3 className="text-base mt-5">Out for Collection</h3>}
        {outdatedOrders
          .filter((order) => order.status === "Out for Sample Collection")
          .map((order) => (
            <OutdatedOrderCard order={order} key={order._id} />
          ))}
        {outdatedOrders.length === 0 && (
          <div className="text-center my-auto text-gray-500">
            No outdated orders
          </div>
        )}
      </div>
...
function OutdatedOrderCard({ order }: { order: Order }) {
  return (
    <div
      key={order._id}
      className="flex items-center gap-4 justify-between px-4 py-2 bg-gray-50 dark:bg-black border-2 border-gray-300/50 dark:border-white/30 rounded-md"
    >
      <div>
