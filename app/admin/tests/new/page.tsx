...
const Page = () => {
  ...
  const [error, setError] = useState<
    { [key in keyof TestDetails]?: string } | null
  >(null);
  ...
  const handleSave = async () => {
    ...
    if (!testDetails.fastingRequired)
      error.fastingRequired = "Fasting requirement is required";
    if (!testDetails.tempOverview) error.overview = "Overview is required";
    if (!testDetails.tempTestResultInterpretation)
      error.testResultInterpretation = "Test result interpretation is required";
    ...
    const res = await fetcher.post("/admin/tests", {
      ...
      testResultInterpretation:
        testDetails.tempTestResultInterpretation ||
        testDetails.testResultInterpretation ||
        "",
      riskAssesment:
        testDetails.tempRiskAssesment || testDetails.riskAssesment || "",
    });
    if (res.status === 200) {
