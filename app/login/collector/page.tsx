...
export default function AdminLogin() {
    ...
    async function login(e: React.MouseEvent<HTMLButtonElement>) {
        ...
        setLoading(true);
        await fetcher.post<{ email: string, password: string }, { user: { verified: boolean, institution: string, type: string }, token: string }>('/collector/auth/login', { email, password: await encryptData(password) }).then(async (res) => {
            if (res.status !== 200) {
