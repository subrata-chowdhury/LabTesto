...
export async function POST(req: NextRequest) {
  try {
    ...
    for (const item of data.items) {
      if (
        ...
        return new NextResponse(
          "Each item must have a product with test, lab, and price",
          { status: 400 },
        );
      }
      if (item.patientDetails?.length > 0)
        for (const patient of item.patientDetails) {
          if (!patient.name || !patient.gender || !patient.age) {
            return new NextResponse(
              "Each patient detail must have name, gender, and age",
              { status: 400 },
            );
          }
        }
    ...
    if (
      ...
      return new NextResponse(
        "Address must have pin, city, district, and phone",
        { status: 400 },
      );
    }
    if (!data.collector) {
      ...
      const collectors = (await Collector.find({
        reachableAreas: { $in: [data.address.pin] },
