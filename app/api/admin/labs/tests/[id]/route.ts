...
export async function GET(req: Request) {
  ...
  try {
    ...
    const tests = (await Test.find({
      [`labsDetails.${lab._id}`]: { $exists: true },
...
export async function POST(req: Request) {
  try {
    ...
    for (const detail of labDetails.details) {
      ...
      if (!test) {
        return NextResponse.json(
          { error: `Test with ID ${detail.test} not found` },
          { status: 404 },
        );
      }
    ...
    return NextResponse.json({
      message: "Lab test details saved successfully",
    });
  } catch (error) {
    console.error(error);
...
export async function DELETE(req: Request) {
  try {
    ...
    if (!test) {
      return new NextResponse(`Test with ID ${body.test} not found`, {
        status: 404,
      });
    }
    if (test.labsDetails.has(id)) {
      ...
      await test.save();
      return NextResponse.json(
        { messege: "Lab test details deleted successfully" },
        { status: 200 },
      );
    } else {
    ...
    console.log(e);
    return new NextResponse("Failed to delete lab test details", {
      status: 500,
    });
  }
}
