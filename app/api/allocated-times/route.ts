...
export async function GET(req: NextRequest) {
  ...
  const collectors = await Collector.find({
    reachableAreas: { $in: [pin] },
  }).lean<ICollector[]>();

  if (!collectors.length) {
    return NextResponse.json(
      { error: "No collectors found for the given pin" },
      { status: 404 },
    );
  }
  ...
  for (let i = 0; i <= 12; i++) {
    ...
    collectorIds.forEach((collectorId) => {
      ...
      uniqueOrders.forEach((order) => {
        if (
          order.collector?.toString() === collectorId &&
          order.sampleTakenDateTime.start?.toTimeString().split(" ")[0] === time
        )
          orderCount++;
      });
      if (orderCount >= 2)
