...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    if (body.review) {
      const reviewIndex = (existingOrder as IOrder).review.findIndex(
        (r) =>
          r.test.toString() === body.review.test &&
          r.lab.toString() === body.review.lab,
      );
      ...
      if (collectorRating) {
        ...
        if (collector) {
          collector.rating =
            (collector.rating * collector.rated + collectorRating) /
            (collector.rated + 1);
          collector.rated += 1;
    ...
    if (existingOrder.status === "Canceled") {
      await fetch(
        "https://api.telegram.org/bot7846622941:AAEjj6UdF2C42GG_S1RVvK2oPhmRxFUCukA/sendMessage",
        {
          method: "POST",
          headers: {
