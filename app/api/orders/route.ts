...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!Array.isArray(body.items) || body.items.length === 0) {
      return new NextResponse("Request body is missing or invalid", {
        status: 400,
      });
    }
    ...
    for (const item of body.items) {
      if (!item.product?.test || !item.product?.lab) {
        return new NextResponse("Product test and lab are required", {
          status: 400,
        });
      }
    ...
    const orderItems: OrderItem[] = await Promise.all(
      body.items.map(
        async (item: {
          product: Product;
          quantity: number;
          address: { pin: string; city: string; phone: string; other?: string };
        }) => {
          ...
          const test = (await Test.findById(item.product.test)
            .select("labsDetails")
            .lean()) as unknown as ITest;
          if (!test) {
    ...
    const collectors = (await Collector.find({
      reachableAreas: { $in: [body.address.pin] },
    }).lean()) as unknown as ICollector[];
    if (collectors.length === 0) {
      return new NextResponse("No verified collectors available", {
        status: 500,
      });
    }
    ...
    await (
      await order.populate("items.product.test")
    ).populate("items.product.lab");
...
Sample Taken Time:
    ...
    return NextResponse.json(
      { message: "Order saved successfully", id: order._id },
      { status: 200 },
    );
  } catch (error) {
    console.error(error);
