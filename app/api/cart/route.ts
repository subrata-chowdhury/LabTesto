...
export async function POST(req: NextRequest) {
  ...
  const priceDetails: { test: string; price: number; offer: number } =
    lab.prices.find(
      (price: { test: string; price: string }) =>
        price.test.toString() === testId,
    )[0];
  const price = (
    priceDetails.price -
    priceDetails.price * (priceDetails.offer / 100)
  ).toFixed(2);
  ...
  const cartData = {
    ...
    items: [
      {
        product: { test: testId, lab: labId, price },
        quantity: 1,
        date: new Date(),
      },
    ],
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const existingItemIndex = existingCart.items.findIndex(
      (item: { product: { test: string; lab: string } }) =>
        item.product.test.toString() === testId &&
        item.product.lab.toString() === labId,
    );
    ...
    existingItem.patientDetails =
      body.patientDetails || existingItem.patientDetails || [];
    if (existingItem.patientDetails.length > body.quantity) {
      existingItem.patientDetails = existingItem.patientDetails.slice(
        0,
        body.quantity,
      );
    }
...
export async function DELETE(req: NextRequest) {
  ...
  try {
    ...
    const itemIndex = cart.items.findIndex(
      (item: { product: { test: string; lab: string }; quantity: number }) =>
        item.product.test.toString() === productDetails.test &&
        item.product.lab.toString() === productDetails.lab &&
