...
export async function POST(request: NextRequest) {
  ...
  if (!name || !email || !password) {
    return new NextResponse("Username, Phone no and password are required", {
      status: 400,
    });
  }
  ...
  if (existingUser) {
    ...
    return new NextResponse("User already exists. Please log in.", {
      status: 400,
    });
  }
  ...
  return NextResponse.json({
    ...
    user: { verified: false, name: newUser.name, email: newUser.email },
    token,
  });
}
