                    ...
                    <div className='grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-8 sm:gap-4 flex-1'>
                            ...
                            <ul className='flex flex-col gap-1'>
                                ...
                                { label: 'Terms & Conditions', href: "/terms-and-conditions" }].map((link, index) => (
                                    <li key={index} className={"mb-1 text-sm" + (isVisible ? " opacity-100 translate-y-0" : " opacity-0 translate-y-6")} style={{ transition: `all 0.5s ease ${0.3 + index * 0.1}s` }}>
                                        <Link href={link.href} className="text-gray-700 hover:text-gray-500 transition-all dark:text-white/75">
                                            {link.label}
                                        </Link>
                                    </li>
                                ))}
                        ...
                        <div className='inline-flex flex-col'>
                            ...
                            <ul className='text-sm text-gray-600d dark:text-gray-400 font-medium flex flex-col gap-1'>
                                <li className={'mb-2 text-gray-700 hover:text-gray-500 transition-all dark:text-white/75 flex gap-2 items-center ' + (isVisible ? " opacity-100 translate-y-0" : " opacity-0 translate-y-6")} style={{ transition: `all 0.5s ease 0.3s` }}><PhoneIcon size={18} /> +91 82507 11212</li>
                                <li className={'mb-2 text-gray-700 hover:text-gray-500 transition-all dark:text-white/75 flex gap-2 items-center ' + (isVisible ? " opacity-100 translate-y-0" : " opacity-0 translate-y-6")} style={{ transition: `all 0.5s ease 0.4s` }}><MailIcon size={18} /> sayan825071das@gmail.com</li>
                                <li className={'mb-2 text-gray-700 hover:text-gray-500 transition-all dark:text-white/75 flex gap-2 items-center ' + (isVisible ? " opacity-100 translate-y-0" : " opacity-0 translate-y-6")} style={{ transition: `all 0.5s ease 0.5s` }}><LocationIcon size={18} /> Kadasole more, Bankura, <br />Pin. 722202 - West Bengal</li>
                            </ul>
                        </div>
                    </div>
                </div>
