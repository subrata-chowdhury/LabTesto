...
export default function AddressDetailsPopup({
  addressDetails,
  onSave,
  onRemove,
  onClose,
  ...
  onSave: (patientDetails: AddressDetails) => void;
  onRemove: () => void;
  onClose: () => void;
}) {
  const [values, setValues] = useState<AddressDetails>(
    addressDetails || {
      pin: "",
      city: "",
      district: "",
      ...
      <div className="px-7 py-4 min-w-80">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4 text-sm pb-4">
          ...
          <Input
            ...
            onChange={(val) => setValues({ ...values, district: val })}
          />
          <Input
            label="Pin *"
            ...
            type="number"
            error={errors.pin}
            onChange={(val) => setValues({ ...values, pin: val })}
          />
          <Input
            label="Phone *"
            value={values.phone}
        ...
        <div className="text-sm flex flex-col gap-1 pt-2">
          ...
          <textarea
            className="border-2 border-gray-300/50 dark:border-white/20 rounded w-full h-20 p-2 outline-none bg-transparent"
            rows={5}
        ...
        <div className="p-5 pb-2 px-0 ms-auto justify-end items-end flex gap-4">
          <div
            className="font-medium text-primary h-10 flex justify-center items-center px-4 border-2 border-primary rounded cursor-pointer"
            onClick={onClose}
          >
            Cancel
          </div>
          <div
            className="bg-primary dark:bg-white/20 font-medium text-white h-10 flex justify-center items-center px-4 rounded cursor-pointer"
            onClick={onRemove}
          >
            Remove
          </div>
