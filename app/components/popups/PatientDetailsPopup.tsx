...
export default function PatientDetailsPopup({
  ...
  onSave: (patientDetails: ValidPatientDetails) => void;
  onRemove: () => void;
  onClose: () => void;
  patients?: PatientDetails[];
}) {
  const [values, setValues] = useState<PatientDetails>(
    patientDetails || {
      name: "",
      gender: "",
      age: "",
      ...
      <div className="px-7 py-4 min-w-80">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 sm:gap-4 text-sm pb-4">
          ...
          <Input
            ...
            type="number"
            error={errors.age}
            onChange={(val) => setValues({ ...values, age: Number(val) })}
          />
        </div>
        <div className="text-sm flex flex-col gap-1 pt-2">
          <label className="font-medium">Other Details / Disability</label>
          <textarea
            className="border-2 border-gray-300/50 dark:border-white/20 bg-transparent rounded w-full h-20 p-2 outline-none"
            rows={5}
            placeholder="Enter Other Details"
            value={values.other}
            onChange={(e) => setValues({ ...values, other: e.target.value })}
          ></textarea>
        </div>
        <div className="p-5 pb-2 px-0 ms-auto justify-end items-end flex gap-4">
          <div
            className="font-medium text-primary dark:text-white h-10 flex justify-center items-center px-4 border-2 border-primary dark:border-white/30 rounded cursor-pointer"
            onClick={onClose}
          >
            Cancel
          </div>
          <div
            className="bg-primary dark:bg-white/15 font-medium text-white h-10 flex justify-center items-center px-4 rounded cursor-pointer"
            onClick={onRemove}
          >
            Remove
          </div>
