  ...
  return (
    <div className="text-sm flex flex-col p-6 pt-5 gap-5">
      <StarReview
        ...
        stars={review.collectorRating}
        onChange={(val) => setReview({ ...review, collectorRating: val })}
      />
      <StarReview
        label="Lab Rating"
        stars={review.labRating}
        onChange={(val) => setReview({ ...review, labRating: val })}
      />
      <StarReview
        label="Platform Rating"
        stars={review.platformRating}
        onChange={(val) => setReview({ ...review, platformRating: val })}
      />
      <div className="flex flex-col gap-1">
        <label className="font-medium">Others</label>
        <textarea
          className="border-2 rounded w-full h-20 p-2 outline-none"
          rows={5}
          placeholder="Enter any other issue or idea that can help you"
