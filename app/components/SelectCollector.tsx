...
const SelectCollector: React.FC<SelectInstituteProps> = ({
  ...
  const onSeach = useCallback(
    debounce(async (name: string) => {
      const res = await fetcher.get<{
        ...
        `/admin/collectors?filter=${JSON.stringify({ email: name })}&limit=5&page=1`,
      );
  ...
  return (
    <div className="relative">
      <input
        className={
          "px-3 py-2 border-black/10 dark:border-white/30 border-2 rounded outline-none w-full " +
          className
        }
        type="text"
        ...
        ref={inputRef}
      />
      {open && collectors.length > 0 && (
        <div className="absolute top-12 left-0 z-10 w-full bg-white dark:bg-black border-2 border-black/10 dark:border-white/30 rounded-md cursor-pointer drop-shadow-lg">
          <div className=" max-h-[150px] overflow-y-auto">
            ...
            {optionElement &&
              collectors.map((option, index) => optionElement(option, index))}
          </div>
        </div>
      )}
    </div>
