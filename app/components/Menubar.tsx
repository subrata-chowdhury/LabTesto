...
const Menubar = () => {
  ...
  return (
    <nav
      ref={ref}
      className={`bg-white dark:bg-black px-6 z-20 fixed w-screen transition-all p-4 shadow-md shadow-primary/5 dark:shadow-black dark:shadow-md`}
      role="navigation"
    ...
    >
      <div className="mx-auto lg:px-4 container flex justify-between items-center">
        ...
        <div
          className={
            "text-white text-lg font-bold mr-0 md:mr-16 transition-all " +
            (isVisible
        ...
        <div className="hidden md:flex gap-6 lg:gap-8 xl:gap-10 xl:ml-5 mr-auto text-primary dark:text-white">
          ...
          >
            Book&nbsp;<span className="hidden lg:block">a Test</span>
          </Link>
          <Link
            href="/about"
            className={
          ...
          >
            Orders&nbsp;<span className="hidden lg:block"></span>
          </Link>
          <Link
            href="/contact"
            className={
        ...
        {isLoggedIn ? (
          <>
            ...
            >
              {(itemCount || 0) > 0 && (
                <div className="absolute -right-1 -top-1 px-1.5 py-0.5 rounded-full text-xs text-white font-medium bg-primary dark:bg-gray-700">
            ...
            <div className="hidden relative md:flex items-center space-x-4 cursor-pointer text-primary">
              ...
              <span onClick={() => setShowProfilePopup((val) => !val)}>
                <UserIcon
                  size={40}
                  className="rounded-full p-2 bg-primary/20 dark:text-white dark:bg-white/25"
                />
              </span>
              {/* </Link> */}
              {showProfilePopup && (
          ...
          <>
            <Link
              href={"/signup"}
              className={
                "px-3 sm:px-5 py-1.5 text-primary dark:text-white font-medium bg-primary/10 dark:bg-white/15 rounded-md text-sm sm:text-base" +
                (isVisible
                  ? " opacity-100 translate-y-0"
                  : " opacity-0 translate-y-6")
              }
...
export function SearchBar({
  ...
  return (
    <div
      className={
        "relative text-sm z-10 dark:text-white flex gap-3 items-center justify-between px-4 py-2 bg-gray-500/5 border border-primary/20 dark:border-white/30 outline-none w-full rounded-full " +
        className
      }
    >
      {showSearchBar && (
        <SelectTest
          ...
          optionElement={(option, index, onClick) => (
            <Link href={"/tests/" + option._id} key={index}>
              <div
                key={index}
                className="px-3 py-2 border-b-2 border-primary/20 dark:border-white/20 dark:bg-black hover:bg-gray-100 dark:hover:bg-white/20"
                onClick={() => {
                  onClick();
                  onSelect(option);
                }}
      ...
      <button
        type="submit"
        className={
          showSearchBar ? "relative right-0 top-0" : "bg-white p-3 rounded-full"
        }
  ...
  return (
    <div className="md:hidden flex flex-col gap-2 fixed left-0 top-0 w-full text-base sm:w-auto px-10 z-20 bg-white dark:bg-black h-screen shadow-2xl">
      <div className="flex relative top-4 mb-6 justify-center items-center">
        <button
          onClick={() => onClose()}
          className="text-primary dark:text-white absolute left-0 cursor-pointer menu focus:outline-none"
        >
          <svg
            className="w-6 h-6 rotate-180"
            fill="none"
      ...
      <Link
        className="flex flex-col gap-2 justify-center items-center cursor-pointer py-5 pb-8 text-primary dark:text-white"
        href={"/profile"}
        onClick={() => onClose()}
      ...
      >
        <UserIcon
          size={80}
          className="rounded-full p-4 bg-primary/20 dark:bg-white/15"
        />
        <div className="text-center">
          <div className="text-primary dark:text-white text-lg font-bold menu">
            {userName ? userName : "Profile"}
          </div>
          <div className="text-primary dark:text-white text-xs font-medium menu">
            {userEmail ? userEmail : ""}
          </div>
        </div>
      </Link>
      <Link
        href="/profile"
        className="flex justify-between bg-primary/10 dark:bg-white/15 hover:bg-black/20 dark:hover:bg-white/20 px-5 rounded-xl text-primary dark:text-white py-2"
        onClick={() => onClose()}
      >
      ...
      <Link
        href="/cart"
        className="flex justify-between bg-primary/10 dark:bg-white/15 hover:bg-black/20 dark:hover:bg-white/20 px-5 rounded-xl text-primary dark:text-white menu py-2"
        onClick={() => onClose()}
      >
        <span className="flex items-center gap-2">
          <FilledCartIcon size={20} />
          Cart
        </span>{" "}
        <span className="ml-12">❯</span>
      </Link>
...
function ProfilePopup({
  ...
  return (
    <div className="flex flex-col gap-1 px-5 py-4 rounded-lg bg-white dark:bg-black shadow-md dark:shadow-black mt-5 border border-primary/25 absolute top-full -right-3 text-primary">
      <UserIcon
        size={80}
        className="rounded-full p-4 mx-auto bg-primary/20 dark:bg-white/25 dark:text-white"
      />
      <div className="text-center mb-4">
        <div className="text-primary dark:text-white text-lg font-bold menu text-nowrap px-2">
          {userName ? userName : "Profile"}
        </div>
        <div className="text-primary dark:text-white text-xs mt-1 font-medium menu">
          {userEmail ? userEmail : ""}
        </div>
      </div>
      <Link
