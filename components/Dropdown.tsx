...
function Dropdown({ options = [], value = "", onChange = () => { }, containerClassName = '', width, height, showPopupAtTop = false, optionElement, ref, loading = false }: Props) {
    ...
    return (
        <div className={'relative ' + containerClassName} style={{ width: width || 'fit-content' }}>
            <div className='flex justify-between items-center border-primary dark:border-gray-400 border-opacity-40 border-2 p-2 rounded cursor-pointer gap-1' onClick={() => !loading ? setIsOpen(!isOpen) : ''} ref={ref} style={{ height: height || 40, width: width || 'fit-content' }}>
            ...
            {isOpen && <div className='border-2 absolute w-full bg-white dark:bg-[#172A46] dark:border-gray-400 z-10 my-1 rounded max-h-60 overflow-y-auto' style={{ top: showPopupAtTop ? 'auto' : '100%', bottom: showPopupAtTop ? '100%' : 'auto' }}>
