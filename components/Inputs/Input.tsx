...
const Input = ({ label = "", placeholder = "", name = "", error = "", value = "", min, max, onChange = () => { }, type = 'text', inputStyle = {}, containerClass = "", labelClass = "", ref }: Props) => {
    return (
        <label className={"flex flex-col gap-1 " + containerClass}>
            ...
            <input type={type} placeholder={placeholder} min={min} max={max} name={name} value={value} onChange={e => onChange(e.target.value)} className="px-3 py-2 border-2 rounded outline-none" style={inputStyle} ref={ref} />
