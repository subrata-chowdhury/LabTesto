...
const PasswordInput = ({ label = "", value = "", placeholder = "", name = "", onChange = () => { }, error = "", iconSize = 24, labelClass = "" }: Props) => {
    ...
    return (
        <label className="flex flex-col gap-1">
            ...
            <div className="flex">
                <input type={isPassword ? "password" : 'text'} placeholder={placeholder} name={name} value={value} onChange={e => onChange(e.target.value)} className="px-3 py-2 border-2 rounded-l outline-none flex-1" />
